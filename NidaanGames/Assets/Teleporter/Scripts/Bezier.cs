...
public class Bezier : MonoBehaviour {
    public BeingHitByRay actorManagerBezier;
    public GameObject interactionPoint;
    public PlayTutorialAudio tutorialAudio;
    public bool endPointDetected;
    public float rayAdjaster = 2f;
    ...
    public string TeleportSprite
    {
        get { return teleportSprite; }
    }
 
    private string teleportSprite;
    ...
    void HandleExtension() {
        ...
        float finalExtension = extendStep + Time.deltaTime * extensionFactor * rayAdjaster;
    ...
    bool CheckColliderIntersection(Vector3 start, Vector3 end) {
        ...
        if (Physics.Raycast(r, out hit, Vector3.Distance(start, end)))
        {
            ...
            else if (hit.collider.tag == "Issue")
            {
                teleportSprite = "Issue";
            }
            else if(hit.collider.tag == "Untagged")
            {
                teleportSprite = "Untagged";
            }
            else if (hit.collider.tag == "UI")
            {
                teleportSprite = "UI";     
            }
            actorManagerBezier = hit.transform.GetComponent<BeingHitByRay>();
            interactionPoint = hit.collider.gameObject;
            tutorialAudio = hit.transform.GetComponent<PlayTutorialAudio>();
            endpoint = hit.point;
            return true;
        }
        else
        {
            actorManagerBezier = null;
        }
